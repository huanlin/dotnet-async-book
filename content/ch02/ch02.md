...
{width=85%}
![圖片來源: http://goo.gl/lQ4qN0](images/thread-theory-practice.png)
...
備註：雖然 `ThreadPool` 類別有提供 `SetMinThreads` 和 `SetMaxThreads` 方法來改變集區大小的下限與上限，但是最好還是別任意使用，因為更改集區大小的預設值往往只會讓效能更糟——除非你非常清楚目前使用的 CLR 版本所實作的執行緒集區的內部運作細節。
